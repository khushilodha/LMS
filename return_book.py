...
class ReturnBook:
    ...
    def return_book_window(self, mainloop):
        ...
        self.return_win.geometry(f"{self.width}x{self.height}+{self.screen_width}+{self.screen_height}")
        ...
        self.top_label = Label(self.return_win, image=self.photo0, bg="#e3b448")
    ...
    def draw(self):
        self.lblEmail = Label(self.return_win, text="EMAIL-ID ", font=("Nirmala UI", 10, 'bold'),fg='black', bg="#cdffcd")
        self.lblEmail.place(x=30,y=80)
        self.lblBookId = Label(self.return_win, text="BOOK ISBN", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#cdffcd")
        self.lblBookId.place(x=300, y=80)
        self.lblIssueDate = Label(self.return_win, text="ISSUE-DATE", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#cdffcd")
        self.lblIssueDate.place(x=30, y=120)
        self.lblDueDate = Label(self.return_win, text="DUE-DATE", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#cdffcd")
        self.lblDueDate.place(x=300, y=120)
        self.lblfine = Label(self.return_win, text="FINE", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#cdffcd")
        self.lblfine.place(x=30, y=160)

        self.emailEntry = Entry(self.return_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.emailEntry.place(x=140,y=80)
        self.bookidEntry = Entry(self.return_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.bookidEntry.place(x=410, y=80)
        self.issueEntry = Entry(self.return_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.issueEntry.place(x=140, y=120, width=145)
        self.returnEntry = Entry(self.return_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.returnEntry.place(x=410, y=120, width=145)

        self.fineEntry = Entry(self.return_win, font=("Nirmala UI", 10, 'bold'), fg='black', bg="white")
        ...
        self.searchbutton = Button(self.return_win, text="Search Book", font=("Nirmala UI", 10, 'bold'), bg="gold",
                                command=self.search_book)
        self.searchbutton.place(x=510, y=200)

        self.returnbutton = Button(self.return_win, text="Return Book", font=("Nirmala UI", 10, 'bold'), bg="gold",
    ...
    def returns_book(self):
        ...
        if self.bookidEntry.get()!='' and self.emailEntry.get()!='':
            ...
            cursor.execute(f"select useremail from issuebook where useremail='{self.emailEntry.get()}' and bookid='{self.bookidEntry.get()}'")
            self.row = cursor.fetchone()
            if self.row != None:
                cursor.execute(f"delete from issuebook where useremail='{self.emailEntry.get()}' and bookid='{self.bookidEntry.get()}'")
                connection.commit()
                connection.close()
                self.showdetails()
                ...
                cur = conn.cursor()
                cur.execute(f"select book_quantity from book where book_isbn='{self.bookidEntry.get()}'")
                row1 = cur.fetchone()

                SQL1 = "update book set book_quantity=%s where book_isbn=%s"
                data1 = (int(row1[0]) + 1, self.bookidEntry.get())
                cur.execute(SQL1, data1)
                conn.commit()
                conn.close()
                self.update_fine()
                messagebox.showinfo("Library Management System", "Book returned successfully.")
                self.clear()
    ...
    def search_book(self):
        ...
        if query1 !='' or query2 !='':
            ...
            if query1 !='' and query2=='':
                ...
                for child in self.trrv.get_children():
                    if query1 == str(self.trrv.item(child)['values'][0]) and query2 == str(self.trrv.item(child)['values'][1]):
                        self.trrv.selection_set(child)
                        self.issueEntry.config(state='normal')
                        self.returnEntry.config(state='normal')
                        self.fineEntry.config(state='normal')
            ...
            messagebox.showerror("Library Management System", "Enter Book Isbn Number or User Email Id.")
    ...
    def treeview(self):
        ...
        self.trrv = ttk.Treeview(self.frame, selectmode="browse", xscrollcommand=self.horscr.set, yscrollcommand=self.verscr.set)
        ...
        for row in result:
            ...
            if delta.days > 0:
                if delta.days <= 10:
                    ...
                    SQL1 = "update issuebook set fine=%s where useremail=%s and bookid=%s"
                    data1 = (10,row[0],row[1])
                    cursor.execute(SQL1, data1)


                elif 10 < delta.days <= 20:
                    SQL2 = "update issuebook set fine=%s where useremail=%s and bookid=%s"
                    data2 = (25,row[0],row[1])
                    cursor.execute(SQL2, data2)

                else:
                    SQL3 = "update issuebook set fine=%s where useremail=%s and bookid=%s"
