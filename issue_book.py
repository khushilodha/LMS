...
class IssueBook
    ...
    def issue_book_window(self, mainloop):
        ...
        self.issue_win.geometry(f"{self.width}x{self.height}+{self.screen_width}+{self.screen_height}")
        ...
        self.top_label = Label(self.issue_win, image=self.photo0, bg="#097969")
    ...
    def draw(self):
        self.lblEmail = Label(self.issue_win, text="EMAIL-ID ", font=("Nirmala UI", 10, 'bold'),fg='black', bg="#E0FFFF")
        self.lblEmail.place(x=30,y=100)
        self.lblBookId = Label(self.issue_win, text="BOOK ISBN", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#E0FFFF")
        self.lblBookId.place(x=300, y=100)
        self.lblIssueDate = Label(self.issue_win, text="ISSUE-DATE", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#E0FFFF")
        self.lblIssueDate.place(x=30, y=140)
        self.lblDueDate = Label(self.issue_win, text="DUE-DATE", font=("Nirmala UI", 10, 'bold'), fg='black', bg="#E0FFFF")
        self.lblDueDate.place(x=300, y=140)

        self.emailEntry = Entry(self.issue_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.emailEntry.place(x=140,y=100)
        self.bookidEntry = Entry(self.issue_win,font=("Nirmala UI", 10, 'bold'),fg='black', bg="white")
        self.bookidEntry.place(x=410, y=100)
        self.issueEntry = DateEntry(self.issue_win, font=('times new roman', 15), date_pattern='yyyy-mm-dd',width=18, state='readonly',fg='#F5F5F5',relief=GROOVE)
        self.issueEntry.place(x=140, y=140, width=145)
        self.returnEntry = DateEntry(self.issue_win, font=('times new roman', 15), date_pattern='yyyy-mm-dd',width=18, state='readonly',fg='#F5F5F5',relief=GROOVE)
        self.returnEntry.place(x=410, y=140, width=145)

        self.searchbutton = Button(self.issue_win, text="Search Book", font=("Nirmala UI", 10, 'bold'), bg="gold",
                                command=self.search_book)
        self.searchbutton.place(x=510, y=200)

        self.issuebutton = Button(self.issue_win, text="Issue Book", font=("Nirmala UI", 10, 'bold'), bg="gold",
    ...
    def issue_book(self):
        ...
        if self.bookidEntry.get()!='' and self.emailEntry.get()!='':
            ...
            cursor.execute(f"select user_email from student where user_email='{self.emailEntry.get()}'")
            ...
            if (self.row == None):
                ...
                cur1.execute(f"select bookid from issuebook where useremail='{self.emailEntry.get()}' and bookid='{self.bookidEntry.get()}'")
                r = cur1.fetchone()
                if(r==None):
                    connection.close()
                    ######
                    ...
                    ##########
                        conn1 = psycopg2.connect(user="postgres",
                                              ...
                                              database="lms")
                        cur1 = conn1.cursor()
                        SQL = "insert into issuebook (useremail, bookid, issued_date, return_date, fine) values(%s, %s, %s, %s, %s)"
                        data = (self.emailEntry.get(), self.bookidEntry.get(), self.issueEntry.get(), self.returnEntry.get(), 0)
                        cur1.execute(SQL,data)
                        conn1.commit()
                        conn1.close()
                        self.showdetails()

                        messagebox.showinfo("Library Management System", "Book issued successfully.")
                    else:
                        messagebox.showinfo("Library Management System", "The quantity of book is zero.")
                        conn.commit()
                        conn.close()
                else:
                    messagebox.showerror("Library Management System", "The book is already issued to this user.")
